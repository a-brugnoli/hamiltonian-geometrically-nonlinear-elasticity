...
class CantileverBeam(Problem):
    ...
    def __init__(self, n_elem_x, n_elem_y, amplitude = 0.01):
        ...
        density = fdrk.Constant(2 * 10**11)
        thickness = 0.002
        young_modulus = fdrk.Constant(7850)
    ...
    def get_initial_conditions(self):
        bend_disp_0 = fdrk.Constant(0)

        mem_velocity_0 = fdrk.as_vector([0, 0])
        bend_velocity_0 = fdrk.Constant(0)

        mem_stress_0 = fdrk.as_vector([[0, 0], [0, 0]])
        bend_stress_0 = fdrk.as_vector([[0, 0], [0, 0]])
    ...
    def get_essential_bcs(self, time_ess):
        ...
        essential_dict = {"bending displacement": {"on_boundary": fdrk.Constant(0)}, \
                        "membrane velocity": {"on_boundary": fdrk.as_vector([0, 0])}, \
