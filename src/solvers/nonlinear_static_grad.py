...
class NonLinearStaticSolverGrad:
    def __init__(self, problem: StaticProblem, pol_degree=2):
        ...
        self.solution_k = fdrk.Function(mixed_space)
        # self.displacement, self.grad_disp, self.first_piola = self.solution.subfunctions
        self.displacement_k, self.grad_disp_k, self.first_piola_k = fdrk.split(self.solution_k)

        self.solution_k.sub(0).assign(fdrk.as_vector([0] * problem.dim))
        ...
        self.loading_factor = 1

        res_equilibrium = fdrk.inner(fdrk.grad(test_disp), self.first_piola_k) * fdrk.dx 
        ...
        res_def_grad = fdrk.inner(test_grad_disp,  fdrk.grad(self.displacement_k) - self.grad_disp_k)*fdrk.dx
        
        res_stress = fdrk.inner(test_first_piola, 
                                problem.first_piola_definition(self.grad_disp_k) - self.first_piola_k) * fdrk.dx
        ...
        D_res_P_DH = fdrk.inner(test_first_piola, 
                                problem.derivative_first_piola(trial_delta_grad_disp, self.grad_disp_k)) * fdrk.dx
    ...
    def plot_displacement(self, axes):
        int_displaced_coordinates = fdrk.Mesh(fdrk.interpolate(self.problem.coordinates_mesh \
                                                               + self.displacement_k, self.disp_space))
