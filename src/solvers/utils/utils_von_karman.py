...
def mass_form_energy(testfunctions, functions, parameters):
...
def dynamics_form_energy(testfunctions, functions, vert_displacement, normal):
    ...
    dynamics_membrane = - fdrk.inner(fdrk.sym.grad(test_mem_velocity), mem_stress) * fdrk.dx \
                        + fdrk.inner(test_mem_stress, fdrk.sym.grad(mem_velocity)) * fdrk.dx 
            
    dynamics_bending = - fdrk.inner(fdrk.grad(fdrk.grad(test_bend_velocity)), bend_stress) * fdrk.dx \
                    + fdrk.inner(test_bend_stress, fdrk.grad(fdrk.grad(test_bend_velocity))) * fdrk.dx \
    ...
    dynamics_coupling = fdrk.inner(test_mem_stress, fdrk.sym(fdrk.outer(fdrk.grad(vert_displacement), fdrk.grad(bend_velocity)))) * fdrk.dx \
        - fdrk.inner(fdrk.sym(fdrk.outer(fdrk.grad(bend_velocity), fdrk.grad(vert_displacement))), bend_stress) * fdrk.dx
...
def functional_energy(time_step, testfunctions, old_states, displacement, parameters):
    """
